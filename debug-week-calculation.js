...
const csvWeekCalculation = targetDate => {
  ...
  while (currentMonday <= thursday) {
    ...
    console.log(
      `  - Checking Monday ${currentMonday.toISOString().slice(0, 10)} (Thursday: ${currentThursday.toISOString().slice(0, 10)})`
    )
...
console.log(
  `May 1, 2025 is a ${['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][may1.getDay()]}`
)
...
for (let day = 1; day <= 31; day++) {
  ...
  while (currentMonday <= thursday) {
    ...
    if (currentThursday.getMonth() === 4 && currentThursday.getFullYear() === 2025) {
  ...
  console.log(
    `May ${day} (${dayName}): Monday ${monday.toISOString().slice(5, 10)}, Thursday ${thursday.toISOString().slice(5, 10)} → Week ${week}`
  )
}
